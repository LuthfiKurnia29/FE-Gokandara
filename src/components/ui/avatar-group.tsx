        ...
        <>
          {avatarItems.slice(0, max).map((child, index) => {
            return cloneElement(child as React.ReactElement<any>, {
              className: cn((child.props as any).className, 'border-2 border-background'),
              style: { marginLeft: index === 0 ? 0 : -spacing, ...(child.props as any).style }
            });
          })}
          ...
          {avatarItems.length > max && (
            <div
              className={cn(
                ...
                (avatarItems[0] as any)?.props?.className
              )}
